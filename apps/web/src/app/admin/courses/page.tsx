                        ...
                        <div className="grid grid-cols-2 gap-3 mb-3">
                          ...
                          <div className="flex gap-2">
                            <input
                              ...
                              placeholder="Video URL or upload video"
                            ...
                            />
                            <label className="relative flex items-center justify-center px-3 py-2 text-xs font-bold text-slate-700 border border-slate-200 bg-slate-50 hover:bg-slate-100 rounded-lg cursor-pointer transition-colors flex-shrink-0">
                              Upload
                              <input
                                type="file"
                                accept="video/*"
                                ...
                                onChange={async (e) => {
                                  ...
                                  try {
                                    ...
                                    const json = await res.json();
                                    if (json.url) setLessonVideo(json.url);
                                  } catch (err) {
                                    console.error(err);
                                  }
                                }}
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-4">
              ...
              <div>
                <label className="block text-xs font-semibold text-slate-700 mb-1.5">Thumbnail Image</label>
                <div className="flex gap-2">
                  <input
                    type="text"
                    ...
                    className="flex-1 px-3 py-2.5 text-sm border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-400 transition-all"
                  />
                  <label className="relative flex items-center justify-center px-4 py-2.5 text-xs font-bold text-slate-700 border border-slate-200 bg-slate-50 hover:bg-slate-100 rounded-xl cursor-pointer transition-colors flex-shrink-0">
                    Upload
                    <input
                      type="file"
                      accept="image/*"
                      ...
                      onChange={async (e) => {
                        ...
                        try {
                          const res = await fetch('/api/admin/upload', { method: 'POST', body: fd });
                          const json = await res.json();
                          if (json.url) setForm({ ...form, thumbnail_url: json.url });
                        } catch (err) {
                          console.error(err);
                        }
                      }}
